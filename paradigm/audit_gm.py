...
class AuditGenerativeModel():
    ...
    def sample_contexts(self, N, N_ctx, mu_rho_ctx, si_rho_ctx, return_pi=False):
        ...
        for s in range(1, N):
            # Markov chain
            ctx[s] = self.sample_next_markov_state(current_state=ctx[s-1], states_values=range(N_ctx), states_trans_matrix=pi_ctx)
    ...
    def generate_batch(self, N_batch=None):
        ...
        for batch in range(N_batch):
            ...
            res = self.generate_run() # *res == rules, rules_long, dpos, timbres, timbres_long, contexts, states, obs (HGM) // contexts, states, obs (NHGM)
            batches.append([*res])
...
class NonHierachicalAuditGM(AuditGenerativeModel):
    ...
    def generate_run(self, N_blocks=None, N_tones=None, mu_rho_ctx=None, si_rho_ctx=None):
        ...
        contexts = self.sample_contexts(N=N_tones, N_ctx=2, mu_rho_ctx=mu_rho_ctx, si_rho_ctx=si_rho_ctx)
...
class HierarchicalAuditGM(AuditGenerativeModel):
    ...
    def sample_rules(self, N_blocks, N_rules, mu_rho_rules, si_rho_rules, return_pi=False):
        ...
        return self.sample_contexts(N_blocks, N_rules, mu_rho_rules, si_rho_rules, return_pi)
    ...
    def sample_timbres(self, rules_seq, N_timbres, mu_rho_timbres, si_rho_timbres):
        ...
        timbres = np.array([self.sample_next_markov_state(current_state=seq, states_values=range(N_timbres), states_trans_matrix=pi_timbre) for seq in rules_seq])
    ...
    def sample_dpos(self, rules, rules_dpos_set):
        ...
        return np.array([self.sample_uniform_choice(rules_dpos_set[rule]) for rule in rules])
    ...
    def generate_run(self, N_blocks=None, N_rules=None, N_tones=None, rules_dpos_set=None, mu_rho_rules=None, si_rho_rules=None, mu_rho_timbres=None, si_rho_timbres=None):
        ...
        rules_long = np.tile(rules[:,np.newaxis], (1, N_tones)) # Store latent rules in a per-tone array # This is equivalent to matlab's repmat
        ...
        timbres_long = np.tile(timbres[:,np.newaxis], (1, N_tones)) # Store timbres in a per-tone array # This is equivalent to matlab's repmat
