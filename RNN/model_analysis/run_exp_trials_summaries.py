...
if __name__ == '__main__':
    ...
    if RUN_LIKELIHOOD_AVERAGES:
        ...
        exp.build_likelihood_summaries(OUTPUT_ROOT, MODEL_NAMES)
        dpos_summaries = exp.build_likelihood_summaries(
            OUTPUT_ROOT, MODEL_NAMES, group_col=HUE_COL)
    ...
    if RUN_DPOS_PROBABILITY:
        ...
        for model_name in MODEL_NAMES:
            ...
            plots.save_figure(fig, avg_path, 'mean_dpos_prob_per_sequence_hist.png')
    ...
    if RUN_ACTIVITY_JOIN:
        ...
        for model_name in MODEL_NAMES:
            ...
            out_csv = out_dir / 'deviant_activity_likelihood_joined.csv'
            pooled.to_csv(out_csv, index=False)
            print(f"  {len(pooled)} trials pooled -> {out_csv.name}")
            ...
            plots.save_figure(fig, out_dir, 'scatter_activity_vs_likelihood.png')
