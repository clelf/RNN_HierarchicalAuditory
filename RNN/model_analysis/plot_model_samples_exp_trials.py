...
if __name__ == '__main__':
    ...
    if random_selection:
        ...
        selected_names = ['sub-seq75_cond13_tau240_mu-0.6_dev1_ses-1_run-1_trials.csv', 'sub-seq79_cond14_tau240_mu-0.6_dev-1_ses-1_run-1_trials.csv']
        selected_files = [trials_path / name for name in selected_names]
    ...
    for f in selected_files:
        obs, cue, ctx, dpos, rule, lim_std, d, tau_std, trial_n = load_trial_sequence(
            f, return_hierarch=True, n_cue_classes=n_cue_classes, cue_seed=11
        )
        # Relabel experimental dpos {2..6} -> model convention {3..7} (see above).
        dpos = dpos + EXPERIMENTAL_DPOS_SHIFT
        obs_list.append(obs)
        cue_list.append(cue)            # (T, n_cue) one-hot
        ctx_list.append(ctx)
    ...
    cue_np = np.stack(cue_list).astype(np.float32)       # (N, T, n_cue) one-hot
    ...
    q = torch.tensor(cue_np, dtype=torch.float32)                # (N, T, n_cue)
    ...
    # Plot
    # =============================================================================
    # plot_samples names figures {save_path}_s{id}.png and randomly picks which batch
    # rows to plot, so a single batched call cannot tie a figure back to its source
    # file. Plot one sequence per call instead, slicing every batched array to a
    # length-1 batch so the figure is named after that sequence's file.
    def _slice_row(x, j):
        return None if x is None else x[j:j + 1]

    for j, src in enumerate(selected_files):
        seq_metrics = {k: _slice_row(v, j) for k, v in sample_metrics.items()}
        seq_params = {k: v[j:j + 1] for k, v in params.items()}
        save_path = output_dir / f"{src.stem}"
